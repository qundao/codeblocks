 ...
 * $HeadURL: svn://svn.code.sf.net/p/codeblocks/code/trunk/src/plugins/compilergcc/compilerMSVC17.cpp $
 */
...
CompilerMSVC17::CompilerMSVC17()
    : Compiler(_("Microsoft Visual C++ 2017"), _T("msvc17"))
{
    m_Weight = 16;
    Reset();
}
...
Compiler * CompilerMSVC17::CreateCopy()
{
    return (new CompilerMSVC17(*this));
}

AutoDetectResult CompilerMSVC17::AutoDetectInstallationDir()
{
    wxString sep = wxFileName::GetPathSeparator();
    wxString idepath;

    // Read the VS2017 environment variable
    wxGetEnv(_T("VS150COMNTOOLS"), &m_MasterPath);
    ...
    if (!m_MasterPath.IsEmpty())
    {
        ...
        if (!sdkfound)
        {
            ...
            wxArrayString vers = GetArrayFromString(wxT("10.0A;8.1A;7.1;7.0A;7.0;6.1;6.0A;6.0"));
